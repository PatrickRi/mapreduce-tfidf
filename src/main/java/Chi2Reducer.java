...
public class Chi2Reducer extends Reducer<Text, Chi2DataMapperArray, Text, Text> {
    ...
    @Override
    protected void reduce(Text category, Iterable<Chi2DataMapperArray> valuesIterablee, Context context) throws IOException, InterruptedException {
        System.out.println("CUSTOM IN R3A " + category.toString());
        List<Chi2DataMapper> values = new ArrayList<>();
        for (Chi2DataMapperArray arr : valuesIterablee) {
            for (Writable val : arr.get()) {
                values.add((Chi2DataMapper) val);
            }
        }
        ...
        for (Writable val : values) {
            ...
            // calc C (sum(c) - A)
            dataMapper.C.set(values.size() - dataMapper.A.get());
            // calc D = N -(A+B+C)
        ...
        context.write(category, new Text(sb.toString()));
    }
